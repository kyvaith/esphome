                                ...
                                void *user_data)
{
    ...
    if (dpi_panel->on_refresh_done &&
        dpi_panel->on_refresh_done(&dpi_panel->base, NULL, dpi_panel->user_ctx)) {
        yield_needed = true;
    }
#endif
    return yield_needed;
...
"""
    ...
    if clock_lane_new not in text:
        if clock_lane_old not in text:
            raise RuntimeError(
                "ESP-IDF DSI clock lane mode line not found; patch needs review"
            )
...
static dma2d_data_burst_length_t jpeg_enc_select_dma2d_burst_length(void)
{
...
"""
    ...
    descriptor_new = (
        ...
        "                   JPEG_DMA2D_EOF_NOT_LAST, dma2d_desc_pixel_format_to_pbyte_value(picture_format),\n"
