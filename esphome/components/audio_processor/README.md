...
| Helper | When to use |
|--------|-------------|
| `create_internal(len, name)` | Audio hot-path ring buffers that must never incur PSRAM bus contention, such as AEC reference rings when the audio task reads/writes them every frame. |
| `create_prefer_psram(len, name)` | Large non-realtime BYTEBUF buffers; tries PSRAM first, falls back to internal. Examples: intercom staging buffers and optional fetch/output/reference rings. |
...
Use raw FreeRTOS `RINGBUF_TYPE_NOSPLIT` only when byte-stream semantics are not
correct. `esp_afe.feed_input_ring_` is the current example: `feed()` needs a
complete AFE frame per item, and the realtime producer must drop a whole frame
rather than split or partially write it.

When an ESPHome BYTEBUF ring carries fixed-size PCM frames, call
`write_without_replacement(..., write_partial=false)` for those frame writes.
The ring can still be a byte stream, but overflow must not leave a partial
frame that shifts every later consumer read.
...
ESP32 only. `ring_buffer_caps.cpp` links against ESP-IDF heap capabilities and
`RingBuffer` from ESPHome core.
