...
class ESPAudioStack : public Component {
 public:
...
#ifdef USE_ESP_AUDIO_STACK_HARDWARE_CODEC
  void configure_gmf_reader_io(uint32_t io_size, uint32_t buffer_size, uint32_t task_stack_size, uint8_t task_priority,
                               uint8_t task_core, bool task_stack_in_psram, bool speed_monitor,
  ...
  void configure_gmf_writer_io(uint32_t io_size, uint32_t buffer_size, uint32_t task_stack_size, uint8_t task_priority,
                               uint8_t task_core, bool task_stack_in_psram, bool speed_monitor,
                               int32_t task_timeout_ms) {
    CodecDevBackend::GmfIoConfig cfg;
    cfg.io_size = io_size;
 ...
 protected:
  ...
  bool i2s_mode_secondary_{false};     // false = primary, true = secondary
