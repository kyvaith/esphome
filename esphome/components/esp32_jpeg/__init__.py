    ...
    cv.Schema(
        {
            ...
            cv.Optional(
                CONF_DECODER_TIMEOUT, default="180ms"
            ): cv.positive_time_period_milliseconds,
            cv.Optional(CONF_DMA2D_AXI_BURSTINESS, default=8): cv.int_range(
                min=1, max=16
            ),
            cv.Optional(CONF_DMA2D_PEAK_LEVEL, default=0): cv.int_range(min=0, max=10),
            cv.Optional(CONF_DMA2D_TRANSACTION_LEVEL, default=1): cv.int_range(
                min=1, max=11
            ),
            cv.Optional(CONF_DMA2D_WRITE_PRIORITY, default=1): cv.int_range(
                min=0, max=3
            ),
            cv.Optional(CONF_DMA2D_READ_PRIORITY, default=1): cv.int_range(
                min=0, max=3
            ),
            cv.Optional(CONF_JPEG_DMA2D_AXI_BURSTINESS, default=1): cv.int_range(
                min=1, max=16
            ),
            cv.Optional(CONF_JPEG_DMA2D_PEAK_LEVEL, default=2): cv.int_range(
                min=0, max=10
            ),
            cv.Optional(CONF_JPEG_DMA2D_TRANSACTION_LEVEL, default=4): cv.int_range(
                min=1, max=11
            ),
            cv.Optional(CONF_JPEG_DMA2D_WRITE_PRIORITY, default=0): cv.int_range(
                min=0, max=3
            ),
            cv.Optional(CONF_JPEG_DMA2D_READ_PRIORITY, default=0): cv.int_range(
                min=0, max=3
            ),
        }
...
async def to_code(config):
    ...
    cg.add_define(
        "CONFIG_ESPHOME_DMA2D_AXI_BURSTINESS", config[CONF_DMA2D_AXI_BURSTINESS]
    )
    cg.add_define("CONFIG_ESPHOME_DMA2D_PEAK_LEVEL", config[CONF_DMA2D_PEAK_LEVEL])
    cg.add_define(
        "CONFIG_ESPHOME_DMA2D_TRANSACTION_LEVEL", config[CONF_DMA2D_TRANSACTION_LEVEL]
    )
    cg.add_define(
        "CONFIG_ESPHOME_DMA2D_WRITE_PRIORITY", config[CONF_DMA2D_WRITE_PRIORITY]
    )
    cg.add_define(
        "CONFIG_ESPHOME_DMA2D_READ_PRIORITY", config[CONF_DMA2D_READ_PRIORITY]
    )
    cg.add_define(
        "CONFIG_ESPHOME_JPEG_DMA2D_AXI_BURSTINESS",
        config[CONF_JPEG_DMA2D_AXI_BURSTINESS],
    )
    cg.add_define(
        "CONFIG_ESPHOME_JPEG_DMA2D_PEAK_LEVEL", config[CONF_JPEG_DMA2D_PEAK_LEVEL]
    )
    cg.add_define(
    ...
    cg.add_define(
        "CONFIG_ESPHOME_JPEG_DMA2D_WRITE_PRIORITY",
        config[CONF_JPEG_DMA2D_WRITE_PRIORITY],
    )
    cg.add_define(
        "CONFIG_ESPHOME_JPEG_DMA2D_READ_PRIORITY", config[CONF_JPEG_DMA2D_READ_PRIORITY]
    )
