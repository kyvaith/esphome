...
template<size_t N>
constexpr HuffmanEncoder make_encoder(const std::array<uint8_t, 16> &counts,
    ...
    if (segment.marker == 0xC4) {
      ...
      while (table_position < payload_size) {
        ...
        if (symbol_count > payload_size - table_position)
          return ESP_ERR_INVALID_ARG;
        if (!parsed->huffman_tables[table_class][table_id].configure(
                counts, payload + table_position, payload_size - table_position))
          return ESP_ERR_INVALID_ARG;
      ...
      for (uint8_t index = 0; index < parsed->component_count; index++) {
        ...
        if (parsed->scan_components[index].dc_table >= HUFFMAN_TABLE_IDS ||
          ...
          return ESP_ERR_NOT_SUPPORTED;
        parsed->needs_normalization |= parsed->scan_components[index].dc_table > 1 ||
                                       parsed->scan_components[index].ac_table > 1;
      }
...
template<size_t N>
void write_dht_table(uint8_t **output, uint8_t table_class, uint8_t table_id,
                     const std::array<uint8_t, 16> &counts, const std::array<uint8_t, N> &symbols) {
...
esp_err_t normalize_huffman_for_hardware(const uint8_t *jpeg, size_t jpeg_size, JpegBuffer *output,
  ...
  *normalized = false;
  ...
  if (source_entropy_size <= (std::numeric_limits<size_t>::max() - header_size - 2) /
                                 FAST_ENTROPY_EXPANSION) {
