...
inline uint8_t *lottie_alloc_pixel_buffer(size_t alloc_bytes, bool *internal) {
  ...
  if (alloc_bytes <= lottie_internal_buffer_max_bytes) {
    ...
    if (largest_internal >= alloc_bytes + lottie_internal_buffer_headroom_bytes) {
      uint8_t *buf = static_cast<uint8_t *>(heap_caps_aligned_alloc(
          lottie_cache_align, alloc_bytes, MALLOC_CAP_INTERNAL | MALLOC_CAP_DMA | MALLOC_CAP_8BIT));
      if (buf != nullptr) {
        if (internal != nullptr) {
          *internal = true;
        }
  ...
  return static_cast<uint8_t *>(
      heap_caps_aligned_alloc(lottie_cache_align, alloc_bytes, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
}
...
inline void lottie_load_task(void *param) {
  LottieContext *ctx = static_cast<LottieContext *>(param);
  ...
  if (!ctx->data_loaded) {
    ...
    if (ctx->data != nullptr) {
      ...
      LV_LOG_TRACE("Lottie loaded embedded source: data=%u bytes", static_cast<unsigned>(ctx->data_size));
    } else if (ctx->file_path != nullptr) {
      lv_lottie_set_src_file(ctx->obj, ctx->file_path);
      LV_LOG_TRACE("Lottie loaded file source: %s", ctx->file_path);
    }
    ...
    if (anim != nullptr) {
      ...
      LV_LOG_TRACE("Lottie anim: data=%u bytes frames=%d..%d total=%d duration=%u ms rate=%d",
  ...
  uint32_t frame_delay_ms = ctx->duration_ms / static_cast<uint32_t>(total_frames);
  ...
  while (!ctx->stop_requested) {
    ...
    uint32_t elapsed_ms = static_cast<uint32_t>((xTaskGetTickCount() - ctx->start_tick) * portTICK_PERIOD_MS);

    int32_t frame;
    if (ctx->loop) {
      uint32_t phase = elapsed_ms % ctx->duration_ms;
      frame = ctx->start_frame + static_cast<int32_t>(static_cast<int64_t>(total_frames) * phase / ctx->duration_ms);
    } else {
    ...
    if (perf_log_enabled) {
      uint32_t render_us = static_cast<uint32_t>(sync_start_us - render_start_us);
      uint32_t sync_us = static_cast<uint32_t>(sync_end_us - sync_start_us);
      perf_frames++;
...
inline bool lottie_launch(LottieContext *ctx) {
  ...
  if (!ctx->pixel_buffer) {
    LV_LOG_ERROR("Pixel buffer alloc failed (%u bytes)", static_cast<unsigned>(alloc_bytes));
    return false;
  }
  memset(ctx->pixel_buffer, 0, alloc_bytes);

  LV_LOG_TRACE("Lottie launch: data=%u bytes size=%ux%u runtime_hidden=%d auto_start=%d",
  ...
  ctx->task_stack =
      static_cast<StackType_t *>(heap_caps_malloc(lottie_task_stack_size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
  ctx->task_tcb =
  ...
  ctx->stop_requested = false;
  ctx->task_handle = xTaskCreateStatic(lottie_load_task, "lottie_anim", lottie_task_stack_size / sizeof(StackType_t),
...
inline void lottie_screen_unload_start_cb(lv_event_t *e) {
  ...
  LV_LOG_TRACE("Lottie task stopped, widget hidden (was_hidden=%d)", static_cast<int>(ctx->runtime_hidden));
}
...
inline bool lottie_init(lv_obj_t *obj, const void *data, size_t data_size, const char *file_path, uint32_t width,
  ...
  LV_LOG_TRACE("Lottie registered (%ux%u), waiting for page load to allocate", static_cast<unsigned>(width),
