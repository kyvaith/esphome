...
inline void svg_render_task(void *param) {
    ...
    if (svg_data == nullptr && ctx->file_path != nullptr) {
        LV_LOG_INFO("Reading SVG from %s ...", ctx->file_path);
    ...
    LV_LOG_INFO("Rendering SVG (%u bytes) to %ux%u ...",
...

    {
        ...
        LV_LOG_INFO("SVG %.0fx%.0f -> %ux%u", ow, oh,
...
inline void svg_free_resources(SvgContext *ctx) {
    ...
    LV_LOG_INFO("SVG PSRAM freed (%ux%u = %u KB)",
             (unsigned)ctx->width, (unsigned)ctx->height,
...
inline bool svg_launch(SvgContext *ctx) {
    ...
    LV_LOG_INFO("SVG render task launched (%u KB PSRAM)",
...
inline void svg_screen_unload_start_cb(lv_event_t *e) {
    ...
    LV_LOG_INFO("SVG task stopped, widget hidden (transition starting)");
}
