...
void LEDCOutput::write_state(float state) {
  ...
  if (fade_duration_ms > 0) {
    ...
    if (ledc_fade_service_installed) {
      ...
      const esp_err_t configure_result = ledc_set_fade_with_time(speed_mode, chan_num, duty, fade_duration_ms);
      const esp_err_t start_result =
          configure_result == ESP_OK ? ledc_fade_start(speed_mode, chan_num, LEDC_FADE_NO_WAIT) : configure_result;
      if (start_result == ESP_OK) {
        this->last_duty_ = duty;
        return;
      }
      ESP_LOGW(TAG, "Unable to start %" PRIu32 "ms hardware fade: %s", fade_duration_ms, esp_err_to_name(start_result));
    }
  }
  if (duty == max_duty) {
